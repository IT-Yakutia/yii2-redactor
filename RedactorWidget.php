...
class RedactorWidget extends Widget
{
    ...
    protected function registerClientScripts()
    {
        ...
        $asset = Yii::$container->get(RedactorAsset::className());
        ...
        $($selector).redactor({
            keyupCallback:function(e)
            {
                ...
                if( (sometext == '@') ){
                    if(e.key == 'Shift' || e.key == '@') return false;
                    if(e.key.length > 1) return false;
                        somename = somename + e.key;
                    showUsers();
                }
                if(sometext == '@' && e.key == ' '){
                    sometext = '';
        ...
        function showUsers(){
            $.ajax({
                url: '/api/users?username='+somename,
                dataType: 'json',
                success: function(data){
                    var list = '';
                    if($(\"#userlistssdfdsf\").length == 0){
                        $(\"body\").append('<ul id=\"userlistssdfdsf\"></ul>');
                        list = document.getElementById(\"userlistssdfdsf\");
                    }
                    else{
                        list = document.getElementById(\"userlistssdfdsf\");
                        list.innerHTML = \"\";
                    ...
                    var posotion = $($selector).closest('.redactor-box').find('div[contenteditable=true]').offset();
                    var left = posotion.left;
                    var top = posotion.top;
                    list.style.display = \"block\";
                    $(list).css({top: top + 30, left: left, position:'absolute'});
                    list.style.backgroundColor = \"#999\";
                    for (var i = 0; i < data.length; i++)
                        $(list).append('<li data-id=' + data[i].id + ' style=\"padding:10px; cursor:pointer;\" onclick=\"console.log(event.target.innerText)\">' + data[i].username + '</li>');
        ...
        //     {
        //         console.log(\"position\", savedRange);
        //     }
        ...
        ", $view::POS_READY, self::INLINE_JS_KEY . $this->options['id']);*/
    }
